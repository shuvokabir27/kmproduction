  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-blue-600 via-blue-600 to-blue-600 px-6 py-3 text-center">
        ...
        <div className="relative p-6 space-y-4 text-center">
          {(offer.image_url || product?.image_url) && !isCombo && (
            <div className="mx-auto w-32 h-32 rounded-2xl overflow-hidden shadow-lg ring-4 ring-slate-300/40">
              <img src={offer.image_url || product?.image_url} alt={offer.title} className="w-full h-full object-cover" />
            </div>
          )}

          <div>
            <h2 className="text-2xl font-extrabold text-slate-900 dark:text-slate-50 leading-tight">{offer.title}</h2>
            {offer.description && <p className="text-sm text-slate-700 dark:text-slate-300 mt-2">{offer.description}</p>}
          </div>

          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-600 text-white font-extrabold text-xl px-6 py-3 rounded-full shadow-lg">
          ...
          {isCombo && comboProducts && comboProducts.length > 0 && (
            <div className="bg-white/70 dark:bg-slate-800/50 rounded-2xl p-3 space-y-2 text-left">
              <div className="text-xs font-bold text-slate-700 dark:text-slate-200 text-center mb-1">এই কম্বোতে যা থাকছে</div>
              ...
              {offer.combo_price && comboOriginalTotal > Number(offer.combo_price) && (
                <div className="text-center text-xs text-slate-600 dark:text-slate-300 pt-1">
                  ...
                  <span className="font-extrabold text-blue-600 dark:text-blue-500">৳{toBn(offer.combo_price)}</span>{" "}
                  <span className="font-bold text-blue-700 dark:text-blue-500">({toBn(Math.round(((comboOriginalTotal - Number(offer.combo_price)) / comboOriginalTotal) * 100))}% সাশ্রয়)</span>
                </div>
              )}
            </div>
          )}
          ...
          {countdown && (
            <div className="inline-flex items-center gap-2 bg-slate-900/90 dark:bg-slate-100 text-slate-300 dark:text-slate-900 font-mono font-bold px-4 py-2 rounded-xl text-sm">
          ...
          <Link
            ...
            onClick={dismiss}
            className="block w-full bg-gradient-to-r from-blue-600 to-blue-600 hover:from-blue-700 hover:to-blue-700 text-white font-bold py-3.5 rounded-2xl shadow-lg hover:shadow-xl transition-all hover:scale-[1.02]"
          >
