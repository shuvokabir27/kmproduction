        ...
        {isCombo ? (
          <>
            <h3 className="text-lg font-bold flex items-center gap-2">
              <Package className="h-5 w-5 text-blue-600" /> কম্বোতে যা যা থাকছে
            </h3>
            ...
            <div className="bg-card border rounded-xl p-4 space-y-2">
              ...
              {comboPrice > 0 && (
                <div className="flex justify-between text-lg">
                  ...
                  <span className="font-extrabold text-blue-600">৳{toBn(comboPrice)}</span>
                </div>
              )}
              {savings > 0 && (
          ...
          <div className="bg-card border rounded-xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-blue-600 font-extrabold text-lg">৳{toBn(Number(productsData[0].discount_price ?? productsData[0].price ?? 0))}</p>
            </div>
          </div>
        ) : null}

        <ul className="space-y-1 text-sm text-muted-foreground pt-2">
          <li className="flex items-center gap-2"><Check className="h-4 w-4 text-blue-600" /> অরিজিনাল পণ্যের নিশ্চয়তা</li>
          <li className="flex items-center gap-2"><Check className="h-4 w-4 text-blue-600" /> ক্যাশ অন ডেলিভারি</li>
          {(offer.combo_free_delivery || isFree) && (
            <li className="flex items-center gap-2"><Check className="h-4 w-4 text-blue-600" /> ডেলিভারি চার্জ ফ্রি</li>
          )}
        </ul>

        {/* Inline Order Form */}
        <div id="offer-order-form" className="bg-card border-2 border-blue-600/30 rounded-2xl p-5 mt-6 shadow-xl">
          {success ? (
            ...
            <>
              <h3 className="text-lg font-extrabold mb-1 flex items-center gap-2">
                <ShoppingBag className="h-5 w-5 text-blue-600" /> অর্ডার করতে নিচের তথ্য দিন
              </h3>
              <p className="text-xs text-muted-foreground mb-4">ক্যাশ অন ডেলিভারি — পণ্য হাতে পেয়ে টাকা দিন</p>

              <div className="space-y-3">
                <div>
                  <label className="text-sm font-bold mb-1.5 flex items-center gap-1.5"><User className="h-4 w-4" /> আপনার নাম <span className="text-blue-600">*</span></label>
                  <input
                    type="text"
                    value={form.name}
                    ...
                    maxLength={100}
                    className="w-full h-11 px-3 rounded-xl border-2 border-border bg-background focus:border-blue-600 outline-none"
                  />
                </div>
                <div>
                  <label className="text-sm font-bold mb-1.5 flex items-center gap-1.5"><Phone className="h-4 w-4" /> মোবাইল নম্বর <span className="text-blue-600">*</span></label>
                  <input
                    type="tel"
                    inputMode="numeric"
                    value={form.phone}
                    ...
                    maxLength={11}
                    className={`w-full h-11 px-3 rounded-xl border-2 bg-background focus:border-blue-600 outline-none ${phoneError ? "border-blue-500" : "border-border"}`}
                  />
                  {phoneError && <p className="text-blue-600 text-xs mt-1">{phoneError}</p>}
                </div>
                <div>
                  <label className="text-sm font-bold mb-1.5 flex items-center gap-1.5"><MapPin className="h-4 w-4" /> ঠিকানা <span className="text-blue-600">*</span></label>
                  <textarea
                    value={form.address}
                    ...
                    rows={3}
                    maxLength={500}
                    className="w-full px-3 py-2 rounded-xl border-2 border-border bg-background focus:border-blue-600 outline-none resize-none"
                  />
                </div>
                ...
                <button
                  ...
                  disabled={submitting}
                  className="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-slate-900 disabled:opacity-60 text-white font-extrabold py-3.5 rounded-2xl shadow-xl text-base"
                >
          ...
          <div className="max-w-3xl mx-auto flex items-center gap-3">
            <div className="flex-1">
              {isCombo && comboPrice > 0 ? (
                <p className="text-xl font-extrabold text-blue-600">৳{toBn(comboPrice)}</p>
              ) : (
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-slate-900 text-white font-extrabold px-6 py-3 rounded-2xl shadow-xl"
            >
              <ShoppingBag className="h-5 w-5" />
              অর্ডার করুন
            </button>
